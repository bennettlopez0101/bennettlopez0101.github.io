...
function getArticle(company) {
    ...
    var pub_date = [];
    var web_url = [];
    var url = "https://api.nytimes.com/svc/search/v2/articlesearch.json?q=" + company +"+&api-key=olyNoXhWtITbUZ0ZgxVvtRproOX7IipG&sort=newest"
    $.ajax({
        url: url,
        type: "GET",
        format: "json",
...
function getPhoto(company) {
    ...
    /* Send the content to the HTML */
    for (var i = 0; i < 3; i++) {
        $(".photo" + (i + 1)).append("<img class = \"artPhotos\" src = \""+ url +"\">");
    }
...
function enableButtons(disable) {
    ...
    // var search = document.getElementbyId("newStock");
    // search.disabled = disable;
    // search.style.color = color;
    // search.style.backgroundColor = backgroundColor;
}
